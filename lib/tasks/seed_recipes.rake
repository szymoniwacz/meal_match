...
namespace :db do
  ...
  task seed_recipes: :environment do
    ...
    all_recipes = english_recipes.map { |r| r.merge('language' => 'english') } +
      french_recipes.map { |r| r.merge('language' => 'french') }

    all_recipes.each do |recipe_data|
      recipe = Recipe.create(
        title: recipe_data['title'] || recipe_data['name'],
        cook_time: recipe_data['cook_time'].to_i,
        prep_time: recipe_data['prep_time'].to_i,
        ratings: recipe_data['ratings'].to_f,
